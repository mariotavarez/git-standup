...
import type { StandupOptions, StandupReport, RepoCommits } from '../types.js';
...
export async function runStandup(cmdOpts: StandupCommandOptions): Promise<void> {
  ...
  const depth =
    typeof cmdOpts.depth !== 'undefined'
      ? Number(cmdOpts.depth)
      : (savedConfig.defaultDepth ?? 2);

  const rawFormat = cmdOpts.format ?? savedConfig.defaultFormat ?? 'full';
  const format = (['full', 'compact', 'markdown'] as const).includes(
    rawFormat as 'full' | 'compact' | 'markdown'
  )
    ? (rawFormat as 'full' | 'compact' | 'markdown')
    : 'full';
  ...
  try {
    ...
    spinnerFind.succeed(
      chalk.dim(`Found ${chalk.bold(repoPaths.length)} git repo${repoPaths.length !== 1 ? 's' : ''}`)
    );
  } catch (err) {
  ...
  const repoCommitsList: RepoCommits[] = [];
  let totalCommits = 0;

  try {
    const commitResults = await Promise.all(
      repoPaths.map((repoPath) => getCommits(repoPath, opts))
    );
    ...
    spinnerCommits.succeed(
      chalk.dim(
        `Found ${chalk.bold(totalCommits)} commit${totalCommits !== 1 ? 's' : ''} across ${chalk.bold(repoCommitsList.length)} active repo${repoCommitsList.length !== 1 ? 's' : ''}`
      )
    );
  } catch (err) {
