...
import type { StandupReport, RepoCommits, CommitInfo, StandupOptions } from '../types.js';
...
function buildHeader(report: StandupReport, author: string | undefined): string {
...
function formatRepoFull(repoCommits: RepoCommits): string {
  const { repoName, commits } = repoCommits;
  const lines: string[] = [];

  lines.push(
    chalk.bold.cyan(`▸ ${repoName}`) +
...
function formatRepoCompact(repoCommits: RepoCommits): string {
  const { repoName, commits } = repoCommits;
...
function buildFooter(report: StandupReport): string {
  ...
  const dateRange =
    uniqueDates.length > 1 ? `${uniqueDates[uniqueDates.length - 1]} → ${uniqueDates[0]}` : uniqueDates[0] ?? '';
...
export function formatFull(report: StandupReport, opts: StandupOptions): string {
  if (report.repos.length === 0) {
    return (
      buildHeader(report, opts.author) +
...
export function formatCompact(report: StandupReport, opts: StandupOptions): string {
  if (report.repos.length === 0) {
    return (
      buildHeader(report, opts.author) +
      chalk.dim('\n  No commits found in the given time range.\n')
...
export function formatMarkdown(report: StandupReport, opts: StandupOptions): string {
  const dateStr = formatDate(report.date);
  const lines: string[] = [];
  ...
  lines.push(
    `---`,
    `_${report.totalCommits} commit${report.totalCommits !== 1 ? 's' : ''} across ${report.repos.length} repo${report.repos.length !== 1 ? 's' : ''}_`
  );

  return lines.join('\n');
}

export function formatReport(report: StandupReport, opts: StandupOptions): string {
  switch (opts.format) {
    case 'compact':
